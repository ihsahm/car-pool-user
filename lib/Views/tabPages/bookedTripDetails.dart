...
class _MyBookedTripsState extends State<MyBookedTrips> {
  ...
  Future<void> getPlaceDirection() async {
    final snapshot = await FirebaseDatabase.instance.reference().child('trips').child(widget.request.tripID).once();
    if (snapshot.snapshot.value != null) {
      Map<dynamic, dynamic>? tripData = snapshot.snapshot.value as Map<dynamic, dynamic>?;
      if(tripData != null) {
        var pickUpLatLng = LatLng(double.parse(tripData['locationLatitude']), double.parse(tripData['locationLongitude']));
        var dropOffLatLng = LatLng(double.parse(tripData['destinationLatitude']), double.parse(tripData['destinationLongitude']));
        showDialog(
            context: context,
        ...
        Marker pickUpLocationMarker = Marker(
            ...
            infoWindow:
            InfoWindow(title: tripData['pickUpLocation'], snippet: "My location"),
        ...
        Marker dropOffLocationMarker = Marker(
            ...
            infoWindow: InfoWindow(title: tripData['destinationLocation'], snippet: "My destination"),
        ...
        child: Row(
          ...
          children: [
            ...
            RatingBarIndicator(
              ...
              rating: double.parse(trip['passengers'][0]) - double.parse(trip['availableSeats']),
              itemSize: 18,
              itemCount: 4,
            ),
          ],
                ...
                Container(
                  child: Column(
                    children: [
                      ...
                      Text(
                          '${estimateDistance(
                              double.parse(trip['locationLatitude']),
                              double.parse(trip['locationLongitude']),
                              double.parse(trip['destinationLatitude']),
                              double.parse(trip['destinationLongitude'])).toStringAsPrecision(2)} Km'),
                    ],
                  ),
                ),
                const SizedBox(
